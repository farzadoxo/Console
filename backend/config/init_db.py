...
def init_db(db_name: str = "db.sqlite3"):
    ...
    try:
        ...
        cursor.execute(f"""INSERT INTO publishers_publisher (id , name , estabilishmenDate , about , logoUrl)
        ...
        cursor.execute(f"""INSERT INTO platforms_platform (id , name , releaseDate , publisher_id , about , picUrl)
                        VALUES ( 1 , 'Playstation5' , '{datetime.date.today().strftime('%Y-%m')}' , 3 , 'This platform created by bill gates in 1998 and release in 2000' , '/'),
                                ( 2 , 'Xbox series X' , '{datetime.date.today().strftime('%Y-%m')}' , 4 , 'This gaming console created by sont co at japan' ,'/')""")
        

        cursor.execute(f"""INSERT INTO games_game (id , title , publishDate , iconUrl , publisher_id , genre_id , esrb_id)
                        VALUES ( 1 , 'Red dead redemption 2' , '{datetime.datetime.now()}' , '/' , 1 , 1 , 2),
                                ( 2 , 'FC25' , '{datetime.datetime.now()}' ,'/' , 2 , 5 , 5)""")
        

        cursor.execute(f"""INSERT INTO tricks_gametrick (id , title , description , createdAt ,game_id , creator_id)
